...
public class TrangChuCtrl {
	...
	public void initialize() {
		...
		setMauMenu(VBTrangChu, List.of());
		setMauMenu(VBHangHoa, List.of("Thuốc", "Thực phẩm chức năng", "Dược mỹ phẩm", "Nhập thuốc", "Đặt thuốc"));
		setMauMenu(VBKhachHang, List.of("Khách hàng", "Khiếu nại & Hỗ trợ"));
		setMauMenu(VBHoaDon, List.of("Hóa đơn", "Đổi - Trả"));
	}

	@SuppressWarnings("unused")
	public void setMauMenu(VBox box, List<String> menuCon) {
		...
		box.setOnMouseClicked(e -> {
			...
			if (VBHienTai == box && isSelected) {
				if (!box.getChildren().isEmpty()
						&& box.getChildren().get(box.getChildren().size() - 1) instanceof VBox) {
			...
			for (String name : menuCon) {
				...
				lbl.setStyle("-fx-padding: 8 5 8 10;" + "-fx-cursor: hand;" + "-fx-font-size: 15px;"
						+ "-fx-font-weight: bold;");
				...
				lbl.setOnMouseClicked(ev -> {
					...
					switch (lbl.getId()) {
					case "Thuốc":
						doiCenterPane(lbl, "/FXML/Thuoc.fxml");
						break;
					}
	...
	public void doiCenterPane(Label lbl, String fxmlPath) {
		try {
